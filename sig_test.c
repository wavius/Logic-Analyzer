...
typedef struct {
    ...
    volatile uint32_t TRIGGER_CFG;   // Offset 0x08
    volatile uint32_t DATA_WINDOW;   // Offset 0x0C
    volatile uint32_t TRIGGER_PTR;   // Offset 0x10
    volatile uint32_t TRIGGER_COUNTS;// Offset 0x14
} LogicAnalyzer_Regs;

// Create a pointer to the hardware base address
#define LA0 ((LogicAnalyzer_Regs *) LOGIC_ANALYZER_0_BASE)
